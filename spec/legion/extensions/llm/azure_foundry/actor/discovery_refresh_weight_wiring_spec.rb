...
RSpec.describe Legion::Extensions::Llm::AzureFoundry::Actor::DiscoveryRefresh do
  ...
  # Catalog world (0.4.0): the routable catalog id is the provider-native
  # offering identity; the base model name rides along as the alias.
  let(:catalog_id) { 'gpt-4o-deployment' }
  let(:base_model_name) { 'gpt-4o' }
  let(:catalog) do
    [{ 'id' => catalog_id, 'model_name' => base_model_name, 'context_window' => 128_000 }]
  end
  let(:instance_cfg) do
    {
  ...
  before do
    ...
    allow(actor).to receive_messages(check_health: ready, fetch_catalog_entries: catalog)
  ...
  def offering_id
    Legion::Extensions::Llm::Inventory::Identity.offering_id(
      instance_key: instance_key, provider_native_key: catalog_id
    )
  end
  ...
  def set_weights(tier: 100, provider: 100, instance: 100, model: nil, offering: nil)
    ...
    provider_config[:models] = { catalog_id => { weight: model } } unless model.nil?
  ...
  describe 'write-time weights on the existing discovery cadence' do
    it 'uses the catalog model id, not the base model name, for offering-scoped weight identity' do
      ...
      expect(catalog_id).not_to eq(base_model_name)
      expect(draft.provider_native_key).to eq(catalog_id)
    ...
    it 'does not replace a freshly observed equivalent two-offering catalog whose order changes' do
      ...
      allow(actor).to receive(:fetch_catalog_entries)
        .and_return(entries, entries.reverse)
      allow(Legion::Extensions::Llm::AzureFoundry).to receive(:discover_instances)
        .and_return({ eastus: instance_cfg })
      original = actor.send(
        :discover_offerings_for_instance, instance_cfg: instance_cfg, instance_key: instance_key
      )
